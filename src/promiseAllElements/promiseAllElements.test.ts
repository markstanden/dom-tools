...
describe.concurrent('promiseAllElements tests:', () => {
    ...
    test.concurrent(`should reject for elements that appear too late`, { timeout: 100 }, async ({ expect }) => {
    ...
    test.concurrent(`should resolve for elements already in the DOM`, async ({ expect }) => {
        ...
        expect.assertions(1);
        expect(promisedElement).resolves.toStrictEqual([testButton1, testButton2]);
    });
    ...
    test(`should return elements that appear after the function call, but within the check window`, async ({ expect }) => {
        ...
        expect.assertions(3);
        await expect(promisedElements).resolves.toStrictEqual([testButton1, testButton2]);
    });
